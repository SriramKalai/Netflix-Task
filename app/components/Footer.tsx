...
const ContentBanner: React.FC<ContentBannerProps> = (props) => {
  return (
    <ul className="text-[13px] list-none">
      {props.content.map((menu, index) => (
        <li key={index} className={`${index === 0 ? '' : 'mt-4'} hover:underline cursor-pointer`}>
          <a>{menu}</a>
        </li>
      ))}
    </ul>
...
const FooterLinks = () => {
  return (
    <div className="flex justify-between flex-col lg:flex-row">
      ...
      />
      <ContentBanner
        content={["Help Centre", "Jobs", "Cookie Preferences"]}
      />
      <ContentBanner
        content={["Gift Cards", "Terms of Use", "Corporate Information"]}
      />
      <ContentBanner
        content={["Media Centre", "Privacy", "Contact Us"]}
      />
    </div>
  )
}
...
const ServiceButton = () => {
  return (
    <div className="mb-6 text-[12px]">
      <button className="p-2" style={{ border: 'solid 1px #808080' }}>Service Code</button>
    </div>
  )
}
...
const Footer = () => {
  return (
    <div className="flex max-w-[980px] px-[4%] mx-auto my-5 flex-col gap-6 text-[#808080] mt-24">
      <SocialMedia />
      <FooterLinks />
      <ServiceButton />
      <span className="text-[11px]">© 1997-2024 Netflix, Inc.‎</span>
    </div>
  )
}
