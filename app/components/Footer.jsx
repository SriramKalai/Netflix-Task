...
const SocialMedia = () => {
  return (
    <div class="flex gap-4 text-white">
      ...
      <a herf="#" aria-label="twitter" class="cursor-pointer">
...
const ContentBanner = (props) => {
  return (
    <ui class="text-[13px] list-none">
      {props.content.map((menu, index) => (
        <li key={index} class={`${index === 0 ? '' : 'mt-4'} hover:underline cursor-pointer`}>
          <a>{menu}</a>
        </li>
      ))}
    </ui>
...
const ServiceButton = () => {
  return (
    <div class="mb-6 text-[12px]">
      <button class="p-2" style={{ border: 'solid 1px #808080' }}>Service Code</button>
    </div>
  )
}
...
const Footer = () => {
  return (
    <div class="flex max-w-[980px] px-[4%] mx-auto my-5 flex-col gap-6 text-[#808080]">
      ...
      <FooterLinks />
      <ServiceButton />
      <span class="text-[11px]">© 1997-2024 Netflix, Inc.‎</span>
    </div>
  )
}
